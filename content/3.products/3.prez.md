...
::KCaseStudy{logo}
The International Organization for Standardization (ISO) produces [Semantic Web](https://en.wikipedia.org/wiki/Semantic_Web) renditions of many of their 
standards' codelists which are published for them by the Open Geospatial Consortium (OGC) as part of a reference data 
Knowledge Graph known as the [Rainbow](https://defs.opengis.net/prez), which is made available online using an instance of Prez.

[Technical Committee 211](https://www.iso.org/committee/54904.html) uses [KGM](https://docs.kurrawong.ai/products/tools/kgm/), our RDF data management application, 
to synchronise changes they make to their may codelists to the OGC's Rainbow.

This is easy for the ISO because KGM contains automated validation and documentation tools.

This is efficient for the OGC because KGM can synchronise parts of a Knowledge Graph with source data, without affecting unrelated parts.

Using KGM helps ensure RDF is well annotated and structured in a way that Prez and work with, so the ISO makes changes 
with confidence, knowing they won't affect data publication.

#title
Case Study: Semantic data management and publication for the ISO

#description
This is an example of how Prez and KGM are used by organisations.
